...
- **kill_only** — es el **nombre de una configuración**, no un mecanismo. `hmm_enabled` y `stacking_enabled` están OFF y XGBoost/vol_overlay ON **porque así lo dice `~/.finanzias/settings.json`**, un archivo que no está en el repo. **Nada los fuerza ni los pisa:** `analysis/technical._toggle` los lee con `default=True`, y `paper_trading/feature_switch.py` —que declara *«always OFF»*— es dead code que sólo importa un harness. **CORREGIDO 2026-09-11 (tarea 181):** acá decía *«forzados OFF … kill_only los pisa»*, y esa frase era, además, la más probable de frenar a quien fuera a investigar el desajuste entre el spec y lo vivo. Lo que sí está declarado en el repo es `analysis.harness_config.HARNESS_MODEL_TOGGLES`, que los productores de artefactos fijan en memoria para no depender de ese archivo.
