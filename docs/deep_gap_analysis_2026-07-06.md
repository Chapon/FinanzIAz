...
- **ACTUALIZACIÓN 2026-07-22 (Chapa):** la premisa *"jamás ejecuta"* quedó **desactualizada**. Es cierta solo para la **cuenta 1 (Sim Principal, manual)**: 0 `atr_tp`. En la **cuenta 2 (Sim Segundo, auto, creada 2026-06-20)** disparó **6×** (JNJ, WELL, SPG, O, PSX, XOM entre 26/06 y 22/07). Mecánica correcta: el engine calcula los exits ATR **antes** que la estrategia y un dedup (`engine.py:638-644`) **descarta el SELL de señal cuando el nivel ATR dispara en el mismo scan** → dentro de un scan el **nivel preempta a la señal**, no al revés. Entre scans distintos la señal sí puede cerrar antes de que el precio alcance el TP (por eso la cuenta manual nunca llega). El `signal_score=1.0` de esas órdenes es un **centinela hardcodeado** (`engine.py:403`, "max conviction"), NO una lectura de la señal.
- **Sweep de `tp_mult` (nuevo, `scripts/tp_mult_sweep_2026-07-22.py`; 4560 entradas neutras 41×10y, salidas ATR-only, mundo C_A4):** el 4.0 **trunca la cola derecha sin beneficio de downside**. Subir el TP es monótonamente mejor y **DD-neutral** (el TP solo toca la punta de arriba; stop/trail gobiernan la de abajo → p5=−7.28% y maxDD≈24.4% idénticos en todos los brazos): mean ret 4.0=1.12% → 6.0=1.28% → sin-TP=1.35%; ret/día 8.7→9.6 bp (mejora **incluso ajustado por slot**, a diferencia de C_A4 §8.3 de Tarea 7). Paired 6.0−4.0 = **+0.165 pts [IC95% +0.10, +0.23]**, positivo o neutro en los 4 regímenes y sin depender de un ticker (sobrevive sin TSLA/NVDA/AMD). Contraste con Tarea 7: aflojar el **trailing** (downside) fue NO-SHIP; aflojar el **TP** (upside) no toca la cola de pérdidas. **Recomendación:** subir `atr_tp_mult` a ~6.0 (o desactivar TP) — pero validar con harness pre-registrado antes de shipear (reglas 2/3). Informe completo: `docs/tp_mult_analysis_2026-07-22.md`.
