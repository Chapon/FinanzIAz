...
El oráculo existe para probar que **el instrumento ve lo que se le pide medir**. La T26 lo especificó mal y la corrida quedó inválida con los 6 criterios pasados. **Cuatro** reglas salen de ahí (la 3 la agregó la T164, y costó dos veredictos publicados):
...
3. **El control igualado tiene que ser una función pura de (semilla, ticker, fecha) — T164.** El de la T26 sorteaba con `(semilla, fecha, **índice de barra**)`, así que **cualquier refresh que mueva el `start` del cohorte lo re-sortea entero**: medido, el **51%** de las decisiones se da vuelta para las mismas fechas al recortar 24 barras de la cabeza. Y sin el ticker en la clave era una **moneda por fecha** —176 de 176 decisiones idénticas entre dos tickers—, o sea que suprimía en bloque el mismo día en toda la cartera: eso no es *«suprimir la misma proporción sin elegir»*, es elegir por fecha, y le infla la varianza del drawdown, que es justo la métrica con la que se lo compara. **Lo que costó:** el `ΔmaxDD` del oráculo contra el control pasó de −15/−17 pp a **−4.30 pp**, cruzó el umbral de −5.00 y dejó **INVÁLIDAS dos corridas con veredicto publicado VÁLIDO** (T37 y T47). Un control que se re-sortea no es un control: es otro experimento cada vez.
