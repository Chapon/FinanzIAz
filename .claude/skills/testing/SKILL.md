...
- **`_cortafuegos_de_red`** (autouse, tarea 209) — corta la red en todo test sin `@pytest.mark.network`. Parchea `socket.socket.connect`/`connect_ex` (el camino de `requests`/`urllib3`/`http.client`: Finnhub, EDGAR, RSS) **y** `curl_cffi.Curl.perform` (el de yfinance 1.x, que **no** pasa por el módulo `socket` — por eso hacen falta los dos). Loopback pasa, así que un servidor local en un test sigue andando. **Y desde la tarea 211 cubre también los subprocesos**: el parche en memoria no se hereda, así que el hijo lo arma solo con `FINANZIAS_BLOQUEAR_RED` + el `sitecustomize.py` de `tests/_cortafuegos` — el mismo camino que la 108 usó para la DB. El marcador `network` exime a las dos mitades (el fixture le borra la variable al hijo). **La implementación vive en `tests/_cortafuegos/cortafuegos_red.py` y es UNA sola**: padre e hijo llaman a la misma `instalar()`, y un test lo fija por identidad — dos copias del mismo corte divergen, que es lo que pasó en la 207.
