...
def test_ascending_curve_no_drawdown():
    ...
    st = compute_drawdown_state(
        130.0, snaps, threshold_pct=0.15, window_days=90, now=NOW
    )
...
def test_valley_triggers_at_threshold():
    ...
    st = compute_drawdown_state(
        85.0, snaps, threshold_pct=0.15, window_days=90, now=NOW
    )
...
def test_just_below_threshold_does_not_trigger():
    ...
    st = compute_drawdown_state(
        85.5, snaps, threshold_pct=0.15, window_days=90, now=NOW
    )
...
def test_rolling_window_ignores_stale_peak():
    ...
    st = compute_drawdown_state(
        90.0, snaps, threshold_pct=0.15, window_days=90, now=NOW
    )
...
def test_no_snapshots_never_triggers():
    st = compute_drawdown_state(
        50.0, [], threshold_pct=0.15, window_days=90, now=NOW
    )
    assert st.drawdown_pct == 0.0
    assert st.triggered is False
...
def test_current_is_new_high():
    ...
    st = compute_drawdown_state(
        120.0, snaps, threshold_pct=0.15, window_days=90, now=NOW
    )
    assert st.drawdown_pct == 0.0
    assert st.triggered is False
...
def test_nonpositive_peak_fails_safe():
    ...
    st = compute_drawdown_state(
        0.0, snaps, threshold_pct=0.15, window_days=90, now=NOW
    )
    assert st.drawdown_pct == 0.0
    assert st.triggered is False
...
def test_none_equities_ignored():
    ...
    st = compute_drawdown_state(
        80.0, snaps, threshold_pct=0.15, window_days=90, now=NOW
    )
    assert st.peak_equity == 100.0
    assert st.n_snapshots_in_window == 1
...
def test_live_account_scenario_does_not_trigger_at_15pct():
    ...
    st = compute_drawdown_state(
        49_257.0, snaps, threshold_pct=0.15, window_days=90, now=NOW
    )
    ...
    st9 = compute_drawdown_state(
        49_257.0, snaps, threshold_pct=0.09, window_days=90, now=NOW
    )
...
def test_threshold_zero_arms_on_any_loss():
    ...
    st = compute_drawdown_state(
        99.0, snaps, threshold_pct=0.0, window_days=90, now=NOW
    )
    assert st.triggered is True
