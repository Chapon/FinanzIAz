...
def _bars(dates, prices):
    return [(d, p, p, p, p) for d, p in zip(dates, prices, strict=True)]
...
def test_evaluate_noship_when_dd_worse():
    summaries, regimes = _base_case(
        summaries={
            "TP_6.0": _summ(0.115, 0.83, 0.26, -0.07),  # maxDD 26% > 20%+0.5
...
def test_evaluate_noship_when_regime_hurts():
    summaries, regimes = _base_case(
        regimes={"TP_6.0": _reg([0.24, -0.28, 0.95, -0.40])}
    )  # bear Δ = -0.30 < -0.05
    v = evaluate(summaries, regimes, dsr=0.9, pbo=0.3)
...
def test_evaluate_noship_when_dcagr_too_small():
    summaries, regimes = _base_case(
        summaries={
            "TP_6.0": _summ(0.101, 0.83, 0.20, -0.07),  # +0.1pp < +0.30pp
...
def test_evaluate_candidate_is_best_sharpe():
    summaries, regimes = _base_case(
        summaries={"TP_6.0": _summ(0.115, 0.70, 0.20, -0.07), "TP_off": _summ(0.130, 0.90, 0.20, -0.07)}
    )  # TP_off mejor Sharpe
    v = evaluate(summaries, regimes, dsr=0.9, pbo=0.3)
