...
def _ramp_bars(n: int, start: float = 100.0, step: float = 1.0) -> list:
    return [(_d(i), start + i * step, start + i * step, start + i * step, start + i * step) for i in range(n)]
...
def test_profit_is_recycled_into_later_entries():
    ...
    bars_by = {"A": bars, "B": bars}
    res = _sim([("A", 5), ("B", 40)], bars_by, max_positions=1, cap_days=10, initial_capital=10_000.0)
    assert res.n_taken == 2
...
def test_filter_zero_suppresses_the_entry():
    bars = _flat_bars(40)
    res = _sim([("A", 5)], {"A": bars}, max_positions=5, entry_filter=lambda _t, _d: 0.0)
...
def test_filter_half_halves_the_notional():
    ...
    half = _sim([("A", 5)], {"A": bars}, max_positions=5, entry_filter=lambda _t, _d: 0.5)
...
def test_filter_never_changes_the_exit_of_a_position_that_was_opened():
    ...
    gated = _sim(
        entries, bars_by, max_positions=5, cap_days=10, entry_filter=lambda t, _d: 0.0 if t == "B" else 1.0
    )
...
def test_counters_add_up():
    ...
    res = _sim(entries, bars_by, max_positions=2, entry_filter=lambda t, _d: 0.0 if t == "T0" else 1.0)
...
def test_rank_score_decides_who_gets_the_scarce_slot():
    ...
    res = _sim(entries, bars_by, max_positions=1, rank_score=lambda t, _d: scores[t])
...
def test_rank_score_ties_break_alphabetically_and_deterministically():
    ...
    forward = _sim([("ZZZ", 5), ("AAA", 5)], bars_by, max_positions=1, rank_score=lambda _t, _d: 0.5)
    backward = _sim([("AAA", 5), ("ZZZ", 5)], bars_by, max_positions=1, rank_score=lambda _t, _d: 0.5)
...
def test_rank_score_never_changes_the_exit_of_a_shared_position():
    ...
    ranked = _sim(
        entries, bars_by, max_positions=2, cap_days=10, rank_score=lambda t, _d: 1.0 if t == "BBB" else 0.0
    )
...
def test_reentry_can_be_re_enabled_for_reproducibility():
    ...
    bars = _flat_bars(60)
    res = _sim([("A", 5), ("A", 7)], {"A": bars}, max_positions=5, cap_days=30, allow_reentry_while_open=True)
    assert res.n_taken == 2
    assert res.n_already_open == 0
...
def test_counters_add_up_with_the_new_rejection_reason():
    ...
    assert (
        res.n_taken + res.n_filtered + res.n_no_slot + res.n_no_cash + res.n_already_open
    ) == res.n_offered
...
def test_cap_days_of_none_keeps_the_global_cap():
    ...
    same = _sim([("A", 5)], {"A": bars}, max_positions=5, cap_days=30, cap_days_of=None)
...
def test_cap_days_of_overrides_the_global_cap_per_position():
    ...
    res = _sim(
        ...
        max_positions=5,
        cap_days=30,
        cap_days_of=lambda t, d: 4 if t == "A" else 30,
    )
...
def test_a_nonsense_cap_falls_back_to_the_global_one():
    ...
    for bad in (0, -5, None, float("nan")):
        res = _sim([("A", 5)], {"A": bars}, max_positions=5, cap_days=7, cap_days_of=lambda t, d, _b=bad: _b)
...
def test_the_hard_cap_closes_winners_too_unlike_the_time_stop():
    ...
    capped = _sim([("A", 5)], {"A": bars}, max_positions=5, cap_days=30, cap_days_of=lambda t, d: 6)
    timed = _sim([("A", 5)], {"A": bars}, max_positions=5, cap_days=30, time_stop_days=6)
...
def test_trail_min_excess_of_none_keeps_the_global_threshold():
    ...
    same = _sim([("A", 2)], {"A": bars}, max_positions=5, cap_days=60, trail_min_excess_of=None)
    assert base.trades[0].held_days == same.trades[0].held_days
...
def test_zero_is_a_valid_threshold_not_a_missing_one():
    ...
    alto = _sim(
        ...
        max_positions=5,
        cap_days=60,
        atr_p=AtrParams(stop_mult=1e9, tp_mult=1e9, trail_mult=2.0, trail_min_excess_atrs=20.0),
    )
    cero = _sim(
        [("A", 2)],
        {"A": bars},
...
def test_it_sees_the_entry_date_not_just_the_ticker():
    ...
    _sim([("A", 2), ("A", 45)], {"A": bars}, max_positions=5, cap_days=10, trail_min_excess_of=_of)
