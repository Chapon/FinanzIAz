...
def test_reconcile_expires_old_pending_orders(test_db):
    ...
    with session_scope() as session:
        session.add(PaperOrder(
            account_id=a.id, ticker="AAPL", side="BUY",
            target_dollars=100, status="pending",
            created_at=cutoff_ago,
        ))
        session.add(PaperOrder(
            account_id=a.id, ticker="MSFT", side="BUY",
            target_dollars=100, status="pending",
...
def test_stamp_order_filled_is_idempotent(test_db):
    ...
    with session_scope() as session:
        ...
        trade = TargetTrade(
            ...
            target_dollars=100, target_shares=1,
            reason="test", source="test",
        )

        # First fill — succeeds, updates the order in place.
        _stamp_order_filled(
            session, acct_db, trade, order,
            fill_price=150.0, fill_shares=1.0,
            commission_paid=0.15, slippage_cost=0.05,
        )
        ...
        _stamp_order_filled(
            session, acct_db, trade, order,
            fill_price=999.0, fill_shares=99.0,
            commission_paid=99.0, slippage_cost=99.0,
        )
        assert order.fill_price == 150.0
