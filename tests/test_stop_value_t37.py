...
def test_con_el_stop_apagado_la_barrera_que_dispara_es_el_trailing():
    ...
    assert atr_exit(current_price=90.0, avg_cost=100.0, high_water_mark=120.0,
                    atr_value=10.0, p=p) == "atr_trail"
    ...
    assert atr_exit(current_price=70.0, avg_cost=100.0, high_water_mark=100.0,
                    atr_value=10.0, p=arm_params(2.0, 2.0)["atr_p"]) == "atr_stop"
    # …y con el stop apagado ese mismo precio NO dispara nada (HWM = entrada ⇒
    # el trailing está suprimido por `trail_min_excess_atrs`).
    assert atr_exit(current_price=70.0, avg_cost=100.0, high_water_mark=100.0,
                    atr_value=10.0, p=p) is None


def test_con_las_dos_apagadas_no_dispara_ninguna_barrera_de_abajo():
    p = arm_params(NO_STOP, NO_STOP)["atr_p"]
    assert atr_exit(current_price=10.0, avg_cost=100.0, high_water_mark=200.0,
                    atr_value=10.0, p=p) is None
...
def _pair(base_map, cand_map, n_resamples=400):
    return regime_criterion(_res(base_map), _res(cand_map),
                            n_resamples=n_resamples, seed=12345)
...
def test_c5_falla_solo_si_el_IC_del_agregado_esta_ENTERO_del_lado_malo():
    base = {"bull_normal": [0.0] * 100, "stress_2018q4": [10.0] * 60,
            "stress_covid_2020": [10.0] * 60, "stress_bear_2022": [10.0] * 60}
    cand = {"bull_normal": [0.0] * 100, "stress_2018q4": [0.0] * 60,
            "stress_covid_2020": [0.0] * 60, "stress_bear_2022": [0.0] * 60}
    c5 = _pair(base, cand)
...
def test_la_tolerancia_se_computa_y_nunca_baja_de_la_material():
    base = {"bull_normal": [1.0, -1.0] * 50, "stress_2018q4": [5.0, -5.0] * 10,
            "stress_covid_2020": [5.0, -5.0] * 10, "stress_bear_2022": [5.0, -5.0] * 10}
    c5 = _pair(base, base)
...
def test_tail_stats():
    r = _R([_T(v / 100.0, "bull_normal") for v in
            [-30.0, -20.0, -10.0] + [1.0] * 97])
    t = tail_stats(r)
    assert t["n"] == 100
...
def _ctx(*, star=(NO_STOP, 2.0), agreement=5, dd_cand=0.30, sharpe_cand=0.58,
         cagr_cand=0.0917, oos_proc=0.0553, oos_base=0.0141, dd_proc=0.229,
         dd_base=0.256, tail_worst_d=0.0, tail_p1_d=0.0, boot=None,
         c5=None, sens5=0.03, sens_close=0.04, c9=(0.01, 0.01, 0.01)):
    cand_arm = arm_name(*star)
    summaries = {
        BASELINE_ARM: {"cagr": 0.0201, "sharpe": 0.20, "max_dd": 0.467,
                       "accounting_ok": True, "exit_mix": {}, "n_taken": 2815},
        cand_arm: {"cagr": cagr_cand, "sharpe": sharpe_cand, "max_dd": dd_cand,
                   "accounting_ok": True, "exit_mix": {}, "n_taken": 2400},
    }
    tails = {BASELINE_ARM: {"worst": -36.2, "p1": -12.1, "p5": -7.0, "n": 2815},
             cand_arm: {"worst": -36.2 + tail_worst_d, "p1": -12.1 + tail_p1_d,
                        "p5": -7.2, "n": 2400}}
    wf = {"star": star, "star_arm": cand_arm, "agreement": agreement,
          "picks": [cand_arm] * agreement, "per_fold": [],
          "proc": {"cagr": oos_proc, "max_dd": dd_proc, "final_equity": 0.0},
          "base": {"cagr": oos_base, "max_dd": dd_base, "final_equity": 0.0}}
    c5 = c5 or {"passes": True, "escalate_c9": False, "escalate_windows": [],
                "tolerance_pts": 1.0, "pooled_delta_pts": 0.4, "windows": {}}
    sweep = {}
    pts = list(C9_POINTS) + [C9_ESCALATION]
    for (rate, depth), d in zip(pts, c9):
        sweep[f"d{depth:.2f}_r{rate:.4f}"] = {
            "rate": rate, "depth": depth, "dcagr_worst": d, "dcagr_mean": d,
            "base_cagr_mean": 0.0, "cand_cagr_mean": 0.0, "per_seed": [],
            "shape": "gradual", "digests_ok": True}
    return dict(summaries=summaries, wf=wf, c5=c5,
                boot=(_Boot() if boot is None else boot), tails=tails,
                sens5={"dcagr": sens5}, sens_close={"dcagr": sens_close},
                sweep=sweep)
...
def test_los_nueve_pasan_y_shipea():
    ...
    for k in ("c1_cagr_oos", "c2_maxdd", "c3_boot", "c4_sharpe", "c5_regime",
              "c6_tail", "c7_folds", "c8_spec", "c9_ruin"):
...
def test_c5_bis_agrega_el_escalon_a_c9_y_puede_darlo_vuelta():
    ...
    c5_esc = {"passes": True, "escalate_c9": True,
              "escalate_windows": ["stress_bear_2022"], "tolerance_pts": 1.0,
              "pooled_delta_pts": -0.2, "windows": {}}
