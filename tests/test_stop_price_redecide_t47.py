...
def _stress(base_vals, cand_vals):
    ...
    def split(vs):
        n = len(vs) // 3
        return {"stress_2018q4": vs[:n], "stress_covid_2020": vs[n:2 * n],
                "stress_bear_2022": vs[2 * n:], BULL_NORMAL: [1.0] * 50}
...
def test_a_single_ugly_window_does_not_block():
    ...
    base = _arm({"stress_2018q4": [0.0] * 30, "stress_covid_2020": [0.0] * 30,
                 "stress_bear_2022": [0.0] * 30, BULL_NORMAL: [1.0] * 50})
    cand = _arm({"stress_2018q4": [-0.5] * 30,      # la ventana fea
...
def test_a_large_and_certain_loss_does_block():
    ...
    base = _arm({r: [0.0] * 40 for r in
                 ("stress_2018q4", "stress_covid_2020", "stress_bear_2022")}
                | {BULL_NORMAL: [1.0] * 50})
    cand = _arm({r: [-5.0] * 40 for r in
                 ("stress_2018q4", "stress_covid_2020", "stress_bear_2022")}
                | {BULL_NORMAL: [1.0] * 50})
    out = regime_criterion(base, cand, n_resamples=300, seed=1)
...
def test_pooled_window_aggregates_the_three_stress_windows():
    base = _arm({"stress_2018q4": [1.0] * 10, "stress_covid_2020": [2.0] * 10,
                 "stress_bear_2022": [3.0] * 10, BULL_NORMAL: [0.0] * 10})
...
def test_detectable_matches_the_helper():
    ...
    assert w["detectable"] == pytest.approx(
        detectable_mean_effect(w["sd_pts"], w["n_base"]))
...
_C5_OK = {"passes": True, "tolerance_pts": 1.0, "material_pts": 1.0,
          "detectable_pts": 0.9, "pooled_delta_pts": 0.1, "pooled_ci_high": 1.0,
          "pooled_ci_low": -0.8, "windows": {}}
...
def test_the_other_five_criteria_are_untouched_from_26b():
    ...
    assert evaluate(_grid(delta=-0.01), _C5_OK, _Boot(0.002),
                    _SENS_OK)["c6_consistency"] is False
