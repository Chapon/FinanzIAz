...
1. **"Done" = los CUATRO comandos en verde, en Windows.** Entorno real: Windows + Anaconda. Antes de declarar terminado:
   ...
   Los tests `network` pegan a Yahoo de verdad y se saltean. **`tests/conftest.py` NO bloquea la red, aunque acá decía que sí (tarea 207):** aísla el log, la DB —incluso en subprocesos—, el fetch de tooltip y Slack, los cuatro autouse; para la red hay que parchear a mano, y `mock_yfinance` es **opt-in** y sólo cubre `data.yahoo_finance.yf`, no `requests`/EDGAR/`feedparser`. Un test desparchado sale a internet de verdad, con las keys vivas de esta máquina. El cortafuegos es la tarea **209**.
