...
from scripts.run_anom_profile_t45 import _delta_samples_pooled, per_trade_pts
...
def _sim(tag: str, entries, bars_by, sigs_by, **kw) -> PortfolioResult:
    return _CACHE.run(f"t51|{tag}", lambda: simulate_portfolio(entries, bars_by, sigs_by, **kw))
...
def oracle_cap_keys(
    cands_by_date: dict[str, list[str]], n_by_date: dict[str, int], realized: dict, *, worst: bool
) -> set[tuple[str, str]]:
    ...
    for d, n in n_by_date.items():
        ...
        if n <= 0 or not pool:
            continue
        ranked = sorted(pool, key=lambda t: (realized.get((t, d), default), t), reverse=not worst)
...
def population_share(base_res: PortfolioResult, n: int, keys: set[tuple[str, str]] | None = None) -> float:
    ...
    if not trades:
        return 0.0
    hit = [t for t in trades if t.held_days >= n and (keys is None or (t.ticker, t.entry_date) in keys)]
...
def dose_response(deltas: dict[int, float], n_star: int) -> dict:
    ...
    changes = sum(1 for a, b in pairwise(nz) if a != b)
    unimodal = changes <= 1
    ...
    return {
        "n_star": n_star,
        "deltas": {str(n): deltas[n] for n in ns},
        "directions": dirs,
        "direction_changes": changes,
        "unimodal": unimodal,
        "neighbour_kept": kept,
        "no_isolated_peak": no_isolated,
...
def evaluate_b(base: dict, cand: dict, boot_base, c6: dict, c8: dict, sens: dict | None) -> dict:
...
def outcome_of(a: dict, b: dict, pop: dict, *, sanity_valid: bool) -> str:
    ...
    if not sanity_valid:
        ...
        why = (
            ""
            if (pop["b_ok"] or pop["a_ok"])
            else " La causa está a la vista en el §5.2: sin población, ni el oráculo "
    ...
    if not pop["b_ok"] and not pop["a_ok"]:
        return (
            "SIN POBLACIÓN — menos del 5% de los trades del baseline llega al "
            "tope. El brazo está SIN PODER, no refutado (mismo diagnóstico que "
            "la T13, §5.2). No hay veredicto."
        )
    if a["ship"]:
        return (
            "SHIP A — el tope condicionado al EVENTO. Ojo con el costo del §7: "
            "el detector no corre en el engine vivo, así que cablearlo pide "
            "construir esa cañería primero."
        )
    if b["ship"]:
        return (
            "SHIP B — el tope INCONDICIONAL. No era el evento, era la tenencia: "
    ...
    if not b["c1_dcagr"] and not a["c1_dcagr"]:
        return (
            "NO-SHIP los dos — el tope de tenencia no aporta sobre el engine de "
            "hoy. La hipótesis del §1 queda refutada CON PODER, que es lo que la "
            "T13 no pudo hacer."
        )
    if not b["c6_dose"]:
        return (
            "NO-SHIP — C6: no hay dosis-respuesta. El efecto vive en un N "
...
def walk_forward(entries, bars_by, sigs_by, common: dict, caps_of: dict, *, tag: str, log=sys.stdout) -> dict:
    ...
    for fi, (train_end, test_lo, test_hi) in enumerate(FOLDS, 1):
        ...
        print(
            f"    [{tag}] fold {fi}/{len(FOLDS)} — train {len(train)} · test {len(test)} …",
            file=log,
            flush=True,
        )
        ...
        r_proc = _sim(
            f"wf|{tag}|{fi}|test|{pick}|eq{proc_eq:.6f}",
            test,
            bars_by,
            sigs_by,
            cap_days_of=caps_of[pick],
...
def regime_pooled(base_res: PortfolioResult, cand_res: PortfolioResult) -> dict:
    ...
    samples = _delta_samples_pooled(xs, ys, n_resamples=REGIME_RESAMPLES, seed=REGIME_SEED)
...
def main(argv: list[str] | None = None) -> int:
    ...
    smoke = bool(
        args.universe != LIVE_UNIVERSE_FILE
        or args.no_walkforward
        or args.no_sensitivity
        or args.seeds != CONTROL_SEEDS
    )
    ...
    print(
        f"Evento A_k{ANOM_K}_m{ANOM_M}: {len(anom_all)} entradas, "
        f"**{len(anom_in_pool)}** dentro del pool del engine "
        f"({len(event_keys)} claves en {len(n_by_date)} días)",
        file=log,
    )
    ...
    grid_pop = announce_grid([t.held_days for t in results[BASELINE_ARM].trades], GRID_N, file=log)
    ...
    for n in GRID_N:
        ...
        results[uncond_name(n)] = _sim(
            f"grid|{uncond_name(n)}", entries, bars_by, sigs_by, cap_days_of=caps_uncond[n], **common
        )
        results[event_name(n)] = _sim(
            f"grid|{event_name(n)}", entries, bars_by, sigs_by, cap_days_of=caps_event[n], **common
        )
    ...
    for k in range(args.seeds):
        ...
        results[name] = _sim(
            f"ctrl|{star_e}|{k}", entries, bars_by, sigs_by, cap_days_of=cap_for_keys(keys, star_e), **common
        )
    ...
    for name, worst in ((ORACLE_ARM, True), (ANTI_ORACLE_ARM, False)):
        ...
        results[name] = _sim(
            f"oracle|{star_e}|{int(worst)}",
            entries,
            bars_by,
            sigs_by,
            cap_days_of=cap_for_keys(keys, star_e),
            **common,
        )
        summaries[name] = summarise(results[name])
    ...
    for fondo, rs in (("alpha", None), ("score", b1)):
        for cap in (20, BASE_CAP):
            ...
            grid22[f"{fondo}_cap{cap}"] = summarise(
                _sim(f"attr|{fondo}|{cap}", entries, bars_by, sigs_by, **kw)
            )["cagr"]
    ...
    if not args.no_sensitivity:
        ...
        s_res: dict[str, Any] = {
            ...
            arm_e: _sim(
                f"sens|{arm_e}", entries, bars_by, sigs_by, cap_days_of=caps_event[star_e], **s_common
            ),
        }
        ...
        sens: dict[str, Any] = {
            ...
            "e_cagr": s_sum[arm_e]["cagr"],
            "b_c1": bool(s_sum[arm_u]["cagr"] - s_sum[BASELINE_ARM]["cagr"] >= KILL_MIN_DCAGR),
            "b_c4": bool(s_boot_u is not None and s_boot_u.ci_low > 0.0),
            "a_c1": bool(s_sum[arm_e]["cagr"] - s_sum[BASELINE_ARM]["cagr"] >= KILL_MIN_DCAGR),
    ...
    sanity = evaluate_sanity(summaries, controls, trade_diff, ctrl_diff_median, repro, pop, star_e)
    vb = evaluate_b(summaries[BASELINE_ARM], summaries[arm_u], boot_b_base, c6_u, c8_u, sens)
    va = evaluate_a(
    ...
    ctx: dict[str, Any] = {
        ...
        "base_cap": BASE_CAP,
        "grid_population": {
            "n_trades": grid_pop.n_trades,
            "mean": grid_pop.mean,
            "p50": grid_pop.p50,
            "p95": grid_pop.p95,
            "max": grid_pop.maximum,
            "arms": [
                {
                    "value": a.value,
                    "n_hit": a.n_hit,
        ...
        "sensitivity": sens,
        "boot": {
            "b_vs_base": _boot_d(boot_b_base),
            "a_vs_base": _boot_d(boot_a_base),
            "a_vs_control": _boot_d(boot_a_ctrl),
            "a_vs_b": _boot_d(boot_a_over_b),
        },
        "verdict_b": vb,
        "verdict_a": va,
        "outcome": outcome,
...
def _control_mean_series(daily: dict, names: list[str]) -> list[tuple[str, float]]:
    ...
    return [(series[0][i][0], statistics.fmean(s[i][1] for s in series)) for i in range(n)]
...
def _report(ctx: dict) -> None:
    ...
    print(f"N* incondicional = {ctx['wf_uncond']['star']} · N* evento = {ctx['wf_event']['star']}")
    ...
    print("\n  Brazos:")
    for name in ["B_base", ctx["arm_uncond"], ctx["arm_event"], "ORACULO_cap", "ANTI_ORACULO_cap"]:
        v = s[name]
        print(
            f"    {name:<16} CAGR {_f(100 * v['cagr'], 7, 2, '%')} · "
    ...
    if band:
        print(
            f"    {'CONTROL x' + str(len(band)):<16} min {100 * band[0]:.2f}% · "
    ...
    for k, ok in sn["checks"].items():
        ...
        print(f"    [{label:<13}] {k}")
    print(
        "    2x2 de atribucion (CAGR): "
        + " . ".join(f"{k} {100 * v:.2f}%" for k, v in rp["grid_2x2"].items())
    )
    print("    cross-check alpha_cap250 (NO es gate): " + rp["states"]["alpha250"])
    p = sn["population"]
    print(
        f"    población del tope — incondicional {100 * p['b_share']:.1f}% "
...
if __name__ == "__main__":
    try:
        ...
        print(f"\n*** presupuesto agotado en {e} — volvé a correr para seguir ***", file=sys.stderr)
