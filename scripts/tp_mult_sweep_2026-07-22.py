...
def regime(d: str) -> str:
    if "2018-10-01" <= d <= "2018-12-24":
        return "stress_2018q4"
    if "2020-02-20" <= d <= "2020-04-07":
        return "stress_covid_2020"
    if "2022-01-03" <= d <= "2022-10-12":
        return "stress_bear_2022"
    return "bull_normal"
...
def simulate(bars, e_idx, tp_mult):
    ...
    p = AtrParams(
        period=14,
        stop_mult=STOP_MULT,
        tp_mult=(tp_mult if tp_mult is not None else 1e9),
        trail_enabled=True,
        trail_min_excess_atrs=1.0,
        trail_mult=None,
    )
    ...
    for i in range(e_idx + 1, last_idx + 1):
        date_i, _o_i, _h_i, _l_i, close_i, _ = bars[i]
        a = atrs[i]
        fired = None
        if a is not None:
            fired = atr_exit(
                current_price=close_i, avg_cost=entry_close, high_water_mark=hwm, atr_value=a, p=p
            )
    ...
    hold = exit_idx - e_idx
    return {"ret": ret, "hold": hold, "reason": exit_reason, "entry_date": entry_date, "daily": daily}
...
def main():
    ...
    for t in UNIVERSE:
        ...
        for e in range(WARMUP, len(bars) - 2, SPACING):
            entries.append((t, e))
    print(
        f"Universo: {len(barcache)} tickers · {len(entries)} entradas neutras (spacing {SPACING}, warmup {WARMUP})\n"
    )
        ...
        rows.append(
            {
                ...
                "payoff": payoff,
                "avg_hold": avg_hold,
                "ret_per_day_bp": mean_ret / avg_hold * 10000,
                "maxDD%": dd * 100,
                "p5%": p5 * 100,
                "tp%": tp_share * 100,
                "trail%": trail_share * 100,
                "stop%": stop_share * 100,
                "cap%": cap_share * 100,
                "n": len(rets),
            }
    ...
    for r in rows:
        print(" | ".join(f"{r[h]:>13.2f}" if isinstance(r[h], float) else f"{r[h]!s:>13}" for h in hdr))
