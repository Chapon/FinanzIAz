...
def build_arms(fill_mode: str = "decision") -> dict[str, dict]:
    ...
    arms[ORACLE_ARM] = {"atr_p": base_p, "eval_mode": "touch",
                        "fill_mode": fill_mode, "stop_filter": _oracle_stop_filter}
    arms[RANDOM_KEEP_ARM] = {"atr_p": base_p, "eval_mode": "touch",
                             "fill_mode": fill_mode,
...
def consistency_across_mults(summaries: dict) -> tuple[int, dict[float, float]]:
    ...
    deltas = {
        m: summaries[arm_name("close", m)]["cagr"] - summaries[arm_name("touch", m)]["cagr"]
        for m in MULTS
    }
...
def evaluate(summaries: dict, regimes: dict, boot) -> dict:
    ...
    return {
        ...
        "c5_regime": c5, "c6_consistency": c6,
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(f"fill_mode = {args.fill_mode}" + (
        ...
        "  (la barrera se llena al precio que tomó la decisión)") + "\n")
    ...
    regimes = {n: regime_trade_breakdown(results[n])
...
def _report(summaries, regimes, verdict, sanity, boot):
    hdr = (f"{'brazo':<18}{'CAGR':>9}{'Sharpe':>9}{'maxDD':>9}{'%stop':>8}"
           f"{'%trail':>8}{'%tp':>7}{'tomad':>7}")
    print(hdr)
    print("-" * len(hdr))
    for mode in MODES:
        for m in MULTS:
            ...
            mark = ("BASE" if n == BASELINE_ARM else
                    "*cand" if n == CANDIDATE_ARM else "")
            print(f"{n:<18}{_f(s['cagr'],9,2,'%')}{_f(s['sharpe'],9,2)}"
    ...
    print(f"  [{'OK  ' if sanity['oracle_quality_ok'] else 'FALLA'}] el harness ve calidad de "
