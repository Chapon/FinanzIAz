...
from scripts.run_stop_cal_replay_t26 import summarise  # noqa: E402
from scripts.run_stop_price_replay_t26b import (  # noqa: E402
    BASELINE_ARM,
    CANDIDATE_ARM,
    CAP_DAYS,
    KILL_DD_TOL,
...
def evaluate(summaries: dict, c5: dict, boot, sens: dict | None) -> dict:
    ...
    if ship:
        outcome = ("SHIP — se cabla `paper_atr_confirm_at_close` con default OFF; "
        ...
        outcome = ("NO-SHIP — C5′: el IC95% del Δ en el agregado de stress está entero "
    ...
    return {
        ...
        "n_consistent": n_consistent, "mult_deltas": mult_deltas,
        "c1_cagr": c1, "c2_maxdd": c2, "c3_boot": c3, "c4_sharpe": c4,
        "c5_regime": c5_ok, "c6_consistency": c6, "c7_sensitivity": c7,
...
def main(argv: list[str] | None = None) -> int:
    ...
    boot = paired_block_bootstrap([r for _, r in daily[BASELINE_ARM]],
                                  ...
                                  seed=BOOT_SEED)
    c5 = regime_criterion(results[BASELINE_ARM], results[CANDIDATE_ARM],
    ...
    port_levels = {n: regime_window_returns(daily[n])
    ...
    if not args.no_sensitivity:
        ...
        s_res = {n: simulate_portfolio(entries, bars_by, sigs_by, **arms[n], **s_common)
                 for n in (BASELINE_ARM, CANDIDATE_ARM)}
        s_sum = {n: summarise(r) for n, r in s_res.items()}
        s_daily = aligned_daily(s_res, [BASELINE_ARM, CANDIDATE_ARM])
        s_boot = paired_block_bootstrap([r for _, r in s_daily[BASELINE_ARM]],
        ...
        sens = {
            ...
            "ci_low": s_boot.ci_low, "ci_high": s_boot.ci_high, "p": s_boot.p_value,
            "c1": (s_sum[CANDIDATE_ARM]["cagr"]
    ...
    ctx = {
        ...
        "verdict": verdict, "sanity": sanity, "c5": c5, "sensitivity": sens,
        "portfolio_delta": port, "portfolio_levels": port_levels,
...
def _report(summaries, ctx, verdict, sanity, boot, c5):
    ...
    for n, s in summaries.items():
        tag = {BASELINE_ARM: "BASE (la regla viva)",
               CANDIDATE_ARM: "*CANDIDATO"}.get(n, "")
        print(f"{n:<18}{_f(s['cagr'],10,2,'%')}{_f(s['sharpe'],9,2)}"
              f"{_f(s['max_dd'],9,1,'%')}{s['n_taken']:>8}  {tag}")

    print("\nSanity (§5):")
    print(f"  [{'OK' if sanity['accounting'] else 'FALLA'}] contabilidad")
    print(f"  [{'OK' if sanity['oracle_quality_ok'] else 'FALLA'}] el oráculo le gana al "
    ...
    for r, w in c5["windows"].items():
        ...
        print(f"{r:<20}{w['n_base']:>6}{w['sd_pts']:>8.2f}{det:>10}"
              f"{w['delta_pts']:>+9.2f}{ci:>20}{star}")
    ...
    for r, st in ctx["portfolio_delta"].items():
        if st["delta"] is None:
            continue
        print(f"  {r:<20} Δ {100*st['delta']:>+8.2f}%  "
    ...
    print(f"Bootstrap pareado: {100*boot.observed:+.2f}pp · IC95% "
          f"[{100*boot.ci_low:+.2f}, {100*boot.ci_high:+.2f}]pp · p={boot.p_value:.3f}")
    if ctx["sensitivity"]:
        s = ctx["sensitivity"]
        print(f"Sensibilidad a {s['max_positions']} slots: ΔCAGR {100*s['dcagr']:+.2f}pp · "
