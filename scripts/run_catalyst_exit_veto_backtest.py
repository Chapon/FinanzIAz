...
from analysis.impact_score import exit_veto_block, imminent_catalyst  # noqa: E402
from analysis.surprise_score import make_surprise_loader  # noqa: E402
        ...
        for ev in events:
            ...
            if signal is not None:
                diagnostics.append({
                    ...
                    "reason": ev.reason,
                    "signal_score": ev.signal_score,
                    "in_gray": (ev.signal_score is not None
                                and vs["gray_low"] <= ev.signal_score <= vs["gray_high"]),
                    "earnings": (earn_loader(ev.ticker).strftime("%Y-%m-%d")
                                 if earn_loader(ev.ticker) else None),
            ...
            sim = replay_event(ev, bars or [], scheduled_exit_idx=sched_idx,
                               cap_days=cap_days, atr_p=atr_p)
            if sim is None:  # sin datos: cae como passthrough no-modificado
                sim = SimExit(event=ev, modified=False, exit_date=ev.sell_date,
                              exit_price=ev.sell_price, exit_reason="no_data",
                              pnl_sim=ev.pnl_real)
            sims.append(sim)

        report = build_report("catalyst_exit_veto", sims, real_curve,
...
def render_md(report, diagnostics, ctx) -> str:
    ...
    L.append("")
    L.append(f"Fecha: {ctx['generated_at'][:10]}. Read-only sobre `{ctx['db']}`. "
             f"Contrafactual: re-evaluar al próximo scan (salida post-earnings, ATR activo).")
    L.append("")
    L.append("## Resultado en una línea")
    L.append("")
    verdict = "**PASA** el kill-criteria → candidato a activar el veto." if ctx["passes_tcat6"] \
        else "**NO pasa** el kill-criteria → el flag queda OFF (dead-code documentado)."
    L.append(f"{verdict} ΔP/L = {report.pnl_delta_pts:+.2f} pts "
    ...
    L.append("")
    L.append(f"- Universo: {ctx['n_sell_events']} SELLs filled "
             f"({ctx['n_signal_sells']} por señal), cuenta {ctx['account_id']}.")
    L.append(f"- Zona gris: [{ctx['veto_settings']['gray_low']}, {ctx['veto_settings']['gray_high']}], "
    ...
    for d in sorted(diagnostics, key=lambda x: x["sell_date"]):
        ...
        L.append(f"| {d['ticker']} | {d['sell_date']} | {ss} | "
    ...
    L.append(f"- Max DD real: {100 * report.max_dd_real:.2f}% · sim: {100 * report.max_dd_sim:.2f}% "
    ...
    L.append("")
    if ctx["passes_tcat6"]:
        L.append("El veto mejora el P/L por encima del umbral sin degradar el DD más de 1.3×. "
        ...
        L.append("El veto no alcanza el umbral de mejora exigido (o degrada el DD). "
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.json:
        print(json.dumps({"report": report.to_dict(), "diagnostics": diagnostics, "ctx": ctx},
