...
from scripts.run_market_regime_r2 import load_spy_bars  # noqa: E402
...
def gate_bites(base: PortfolioResult, cand: PortfolioResult) -> dict:
    ...
    return {
        "trade_diff": trade_diff, "capital_diff": cap_diff,
        # Descriptivos (NO son el criterio): qué fracción de las entradas del
        # candidato cayó en risk-off y por lo tanto entró achicada.
        "scaled_trade_share": scaled_share, "scaled_capital_share": scaled_capital,
        "ok": (trade_diff >= SANITY_MIN_TRADE_DIFF
               or cap_diff >= SANITY_MIN_CAPITAL_DIFF),
    }
...
def evaluate(base: dict, cand: dict, reg_base: dict, reg_cand: dict, boot,
             rb: dict, pbo_val: float | None, sens: dict | None) -> dict:
    ...
    if ship:
        outcome = ("SHIP — el detector entra como fuente de leads con el overlay de "
                   "régimen de T20 aplicado, APAGADO detrás de un flag hasta que "
                   "Chapa lo prenda (§7).")
    elif c1 and not c2:
        outcome = ("NO-SHIP — C2: el gate no arregla el bear, que era el ÚNICO motivo "
                   "por el que T11b no shipeó. Si no lo arregla, no sirve para nada.")
    elif c2 and not c1:
        outcome = ("NO-SHIP — C2 pasa y C1 falla: el gate arregla el bear pero cuesta "
    ...
    return {
        ...
        "c1_cagr": c1, "c2_regime": c2, "c2_tolerance": c2_tol, "c2_strict": c2_strict,
        "c3_maxdd": c3, "c4_sharpe": c4, "c5_bootstrap": c5, "c6_pbo": c6,
        "c7_sensitivity": c7, "ship": ship, "outcome": outcome,
    }
...
def main(argv: list[str] | None = None) -> int:
    ...
    entries = build_anomaly_entries(bars_by, vol_by,
                                    AnomalyParams(k=SIGNAL_K, m=SIGNAL_M),
                                    warmup=args.warmup)
    if not entries:
    ...
    def run_gate(cfg: dict, entries_, **over) -> PortfolioResult:
        ...
        return simulate_portfolio(entries_, bars_by, sigs_by,
                                  entry_filter=filt, **dict(common, **over))
    ...
    rand_dist = random_baseline(run_plain, bars_by, count_by_month, operable_by_month,
    ...
    oracle_ret = precompute_oracle_returns(
        operable, bars_by, sigs_by, atr_p=AtrParams(), so_params=ScaleOutParams(),
    ...
    top = sorted((ti for ti, _ in op_scored[:len(entries)]),
    ...
    daily = aligned_daily(results, [BASELINE_ARM] + [n for n in GATE_ARMS
                                                     if n != BASELINE_ARM])
    ...
    if not args.no_sensitivity:
        ...
        s_res = {n: run_gate(GATE_ARMS[n], entries,
        ...
        sens = {
            ...
            "c1_sign": (s_sum[CANDIDATE_ARM]["cagr"]
                        - s_sum[BASELINE_ARM]["cagr"]) >= KILL_MIN_DCAGR,
    ...
    sanity["all_ok"] = bool(sanity["accounting"] and sanity["oracle_ok"]
                            and sanity["edge_survives"] and bites["ok"])

    verdict = evaluate(summaries[BASELINE_ARM], summaries[CANDIDATE_ARM],
                       reg_by[BASELINE_ARM], reg_by[CANDIDATE_ARM], boot, rb,
                       (pbo.pbo if pbo else None), sens)
    if not sanity["all_ok"]:
        ...
        if not sanity["edge_survives"]:
            ...
            verdict["outcome"] = ("CORRIDA INVÁLIDA — falla un sanity del §5; no hay "
    ...
    others = {n: summaries[n]["cagr"] for n in GATE_ARMS
              if n not in (BASELINE_ARM, CANDIDATE_ARM)}
    ctx = {
        ...
        "random_baseline": rb, "sanity": sanity, "verdict": verdict,
        "bootstrap": vars(boot), "pbo": (pbo.pbo if pbo else None), "obs": T,
        "regimes": reg_by, "regime_n_trades": n_trades_by,
        "regime_per_trade": per_trade_by,
        "secondary_cagr": others, "sensitivity": sens,
    }
...
def _report(summaries, ctx, verdict, sanity, boot, rb):
    ...
    print(hdr)
    print("-" * len(hdr))
    tags = {BASELINE_ARM: "BASE (T11b tal cual)", CANDIDATE_ARM: "*CANDIDATO primario",
            ORACLE_ARM: "sanity"}
    for n, s in summaries.items():
        print(f"{n:<16}{_f(s['cagr'],10,2,'%')}{_f(s['sharpe'],9,2)}"
              f"{_f(s['max_dd'],9,1,'%')}{s['n_taken']:>8}{_f(s['exposure'],8,1,'%')}"
              f"  {tags.get(n, 'secundario (descriptivo)')}")
    print(f"{'AZAR_TIME_MATCH':<16}{_f(rb['cagr_median'],10,2,'%')}"
    ...
    print(f"  [{'OK' if sanity['edge_survives'] else 'FALLA'}] el edge de T11b sobrevive: "
          f"baseline {_f(summaries[BASELINE_ARM]['cagr'],0,2,'%')} vs p95 del azar "
          f"{_f(rb['cagr_p95'],0,2,'%')}")
    b = sanity["gate_bites"]
    print(f"  [{'OK' if b['ok'] else 'FALLA'}] el gate muerde: {100*b['trade_diff']:.1f}% "
          f"de trades distintos o {100*b['capital_diff']:.1f}% del capital (mín 10%)")
    print(f"       descriptivo (NO es el criterio): el gate achicó "
    ...
    for r in [BULL_NORMAL] + [x.name for x in STRESS_REGIMES]:
        ...
        print(f"  {r:<20} base {100*rb_:>+8.2f}% (n={nb:>3}) · cand {100*rc_:>+8.2f}% "
              f"(n={nc:>3}) · Δ {100*(rc_-rb_):>+7.2f}pp{star}")

    print("\nMISMO eje, métrica POR TRADE (descriptivo — es con la que T11b midió su "
          "fallo de régimen):")
    for r in [BULL_NORMAL] + [x.name for x in STRESS_REGIMES]:
    ...
    if ctx["sensitivity"]:
        s = ctx["sensitivity"]
        print(f"Sensibilidad a {s['max_positions']} slots: ΔCAGR "
              f"{100*s['dcagr']:+.2f}pp · Δbear {100*s['bear_delta']:+.2f}pp")
    print(f"Secundarios (descriptivos, NO promovibles): "
          + " · ".join(f"{n} {100*v:.2f}%" for n, v in ctx["secondary_cagr"].items()))

    print("\nRegla de decisión (§6):")
    for k, label in [("c1_cagr", "C1 ΔCAGR ≥ 0.00pp (no puede costar retorno)"),
    ...
    print(f"\n  PBO = {ctx['pbo'] if ctx['pbo'] is None else round(ctx['pbo'], 3)} "
          f"(T={ctx['obs']} obs)")
