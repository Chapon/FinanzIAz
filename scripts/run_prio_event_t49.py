...
from scripts.run_anom_profile_t45 import regime_criterion  # noqa: E402
from scripts.run_anomaly_replay_t11b import _pct, load_bars_signals_volume  # noqa: E402
from scripts.run_anom_profile_t45 import merge_entries  # noqa: E402
from scripts.run_rank_neutral_t39 import aligned_daily, policy_series  # noqa: E402
...
def oracle_prio_keys(cands_by_date: dict[str, list[str]], n_by_date: dict[str, int],
                     realized: dict, *, best: bool) -> set[tuple[str, str]]:
...
def evaluate(base: dict, cand: dict, controls: list[dict], boot_base, boot_ctrl,
             c6: dict, sens: dict | None) -> dict:
    ...
    if ship:
        outcome = ("SHIP — se cabla `paper_event_priority_enabled` con default OFF; "
                   "prenderlo es decisión de Chapa (§7). Toca decisiones vivas de "
                   "ENTRADA.")
    elif c1 and c4 and not (c2 and c5):
        outcome = ("NO-SHIP — C2/C5: el candidato le gana al desempate vivo pero NO "
...
def evaluate_sanity(summaries: dict, controls: list[dict], trade_diff: float,
    ...
    seeds_ok = bool(ctrl_diff_median >= SANITY_MIN_TRADE_DIFF
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(f"Anomalía A_k{ANOM_K}_m{ANOM_M}: {len(anom_all)} entradas, de las cuales "
          ...
          f"pool nuevo, que es lo que la 45 rechazó por C8)", file=log)
    print(f"Días con prioridad: {len(n_by_date)}\n", file=log)

    common = _common(args.max_positions, args.capital, CAP_DAYS)

    def run(rank, **over):
        return simulate_portfolio(entries, bars_by, sigs_by,
                                  **{**common, **over}, rank_score=rank)
    ...
    results: dict[str, PortfolioResult] = {
        ...
        ALPHA_ARM: run(None),
        ORACLE_ARM: run(make_prio(
            oracle_prio_keys(cands_by_date, n_by_date, realized, best=True), score_by)),
        ANTI_ORACLE_ARM: run(make_prio(
            oracle_prio_keys(cands_by_date, n_by_date, realized, best=False), score_by)),
    }
    ...
    results[MERGED_ARM] = simulate_portfolio(
        merged, bars_by, sigs_by, **common,
        rank_score=make_prio(keys_of(anom_all, bars_by), score_by))
    ...
    boot_base = paired_block_bootstrap(
        [r for _, r in daily[BASELINE_ARM]], [r for _, r in daily[CANDIDATE_ARM]],
    ...
    boot_ctrl = paired_block_bootstrap(
        [r for _, r in ctrl_series], [r for _, r in daily[CANDIDATE_ARM]],
        block=BOOT_BLOCK, n_resamples=args.resamples, seed=BOOT_SEED)
    ...
    c6 = regime_criterion(control_pts, results[CANDIDATE_ARM],
    ...
    pair_diffs = [trade_overlap(results[ctrl_names[i]], results[ctrl_names[j]])
                  for i in range(len(ctrl_names)) for j in range(i + 1, len(ctrl_names))]
    ...
    if not args.no_sensitivity:
        ...
        def s_run(rank):
            return simulate_portfolio(entries, bars_by, sigs_by, **s_common,
                                      rank_score=rank)

        s_base = summarise(s_run(b1))
        s_cand = summarise(s_run(make_prio(prio_keys, score_by)))
        s_ctrl = [summarise(s_run(make_prio(
            rate_matched_priority(cands_by_date, n_by_date, CONTROL_SEED_BASE + k),
            score_by))) for k in range(args.seeds)]
        s_p95 = _pct([c["cagr"] for c in s_ctrl], KILL_CONTROL_PCTILE)
        sens = {
            "max_positions": args.sens_max_positions,
            "base_cagr": s_base["cagr"], "cand_cagr": s_cand["cagr"],
            "control_p95": s_p95,
    ...
    if args.no_repro:
        ...
        r_merged = summarise(simulate_portfolio(
            merged, bars_by, sigs_by, **r_common,
            rank_score=make_binary_prio(keys_of(anom_all, bars_by))))
        t33 = summarise(simulate_portfolio(
    ...
    ctx = {
        ...
        "n_anom_all": len(anom_all), "n_anom_in_pool": len(anom_in_pool),
        "n_prio_days": len(n_by_date), "max_positions": args.max_positions,
        "seeds": args.seeds,
        ...
        "regime_criterion": c6, "sensitivity": sens, "repro": repro,
...
def _report(summaries: dict, ctrl_names: list[str], ctx: dict) -> None:
    ...
    for n in (BASELINE_ARM, CANDIDATE_ARM, ALPHA_ARM, MERGED_ARM,
              ORACLE_ARM, ANTI_ORACLE_ARM):
        s = summaries[n]
        tag = {BASELINE_ARM: " BASE", CANDIDATE_ARM: " *CAND"}.get(n, "")
        print(f"{n:<20}{_f(s['cagr'],9,2,'%')}{_f(s['sharpe'],9,2)}"
    ...
    print(f"  → el candidato da {_f(summaries[CANDIDATE_ARM]['cagr'],0,2,'%')} "
    ...
    print(f"                               bootstrap pareado vs CONTROL "
    ...
    wh = (f"  {'ventana':<18}{'n cand':>7}{'σ':>7}{'detect':>8}{'Δ vs control':>14}"
          f"{'IC95%':>20}")
    ...
    print(f"  turno muerde {100*sa['trade_diff']:.1f}% · semillas del control "
          f"{100*sa['ctrl_diff_median']:.1f}%")
    print(f"  corrida {'VÁLIDA' if sa['valid'] else 'INVÁLIDA'}")

    print("\nCriterios (§6):")
    for k in ("c1_dcagr", "c2_vs_control", "c3_maxdd", "c4_boot_base",
              "c5_boot_control", "c6_regime", "c7_sensitivity"):
