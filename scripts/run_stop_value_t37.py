...
class SimCache:
    ...
    * **El temporal es único por proceso.** El ``.tmp`` de nombre fijo era el
    ...
    def _file(self, tag: str) -> Path | None:
        if not self.path:
            return None
        h = hashlib.sha256(f"{CACHE_VERSION}|{tag}".encode()).hexdigest()[:32]
...
def never_armed_trailing(
    res: PortfolioResult, bars_by: dict, trail_min_excess_atrs: float = 1.0, period: int = 14
) -> dict:
...
def regime_criterion(base: PortfolioResult, cand: PortfolioResult, *, n_resamples: int, seed: int) -> dict:
    ...
    **C5′-bis** (enmienda §3-bis): además marca si alguna ventana de stress
    ...
    for r in (*REGIMES, "stress_POOLED"):
        ...
        windows[r] = {
            ...
            "resolves_negative": bool(
                delta < 0 and det is not None and math.isfinite(det) and abs(delta) >= det
            ),
        }
    ...
    return {
        ...
        "passes": passes,
        "windows": windows,
        "escalate_c9": bool(escalate),
        "escalate_windows": escalate,
    }
...
def walk_forward(entries, bars_by, sigs_by, common: dict, *, tag: str = "", log=sys.stdout) -> dict:
    ...
    for fi, (train_end, test_lo, test_hi) in enumerate(FOLDS, 1):
        ...
        for s, t in cells:
            r = _sim(
                f"wf{fi}|train|{arm_name(s, t)}|{tag}", train, bars_by, sigs_by, **arm_params(s, t), **common
            )
        ...
        r_proc = _sim(
            f"wf{fi}|test|{arm_name(*pick)}|eq{proc_eq:.6f}|{tag}",
            test,
            bars_by,
            sigs_by,
            **arm_params(*pick),
        ...
        r_base = _sim(
            f"wf{fi}|test|{BASELINE_ARM}|eq{base_eq:.6f}|{tag}",
            test,
            bars_by,
            sigs_by,
    ...
    return {
        "per_fold": per_fold,
        "picks": [arm_name(*p) for p in picks],
        "star": star,
        "star_arm": arm_name(*star),
    ...
    *,
    ...
    for rate, depth in points:
        ...
        for sd in use_seeds:
            ...
            world, events = inject_ruin(bars_by, rate=rate, depth=depth, shape=shape, seed=sd)
            digest = bars_digest(world)
            # §7.6 — determinismo del mundo, verificado en el momento.
            again, _ = inject_ruin(bars_by, rate=rate, depth=depth, shape=shape, seed=sd)
            digest_ok = bars_digest(again) == digest
            rb = _sim(f"ruin|{wtag}|base|{tag}", entries, world, sigs_by, **base_p, **common)
            rc = _sim(
                f"ruin|{wtag}|cand{arm_name(*candidate)}|{tag}", entries, world, sigs_by, **cand_p, **common
            )
            per_seed.append(
                {
                    "seed": sd,
        ...
        print(
            f"    ruina {shape} d={depth:.0%} r={rate:.2%} -> "
            f"D medio {100 * out[key]['dcagr_mean']:+.2f} pp / "
            f"peor {100 * out[key]['dcagr_worst']:+.2f} pp",
            file=log,
            flush=True,
        )
    return out
...
def breakeven_rate(sweep: dict, depth: float) -> float | None:
    ...
    """
    pts = sorted(
        ((v["rate"], v["dcagr_worst"]) for v in sweep.values() if abs(v["depth"] - depth) < 1e-12),
        key=lambda x: x[0],
    )
    if not pts or pts[0][1] < 0:
        return 0.0 if pts else None
    for (r0, d0), (r1, d1) in pairwise(pts):
        if d1 < 0 <= d0:
            if d0 == d1:
                return r1
...
def ruin_dose_response(sweep: dict, depth: float = 0.50) -> dict:
    ...
    **(b) Dosis-respuesta con tolerancia COMPUTADA:** para cada par consecutivo de
    ...
    """
    pts = sorted(
        ((v["rate"], v) for v in sweep.values() if abs(v["depth"] - depth) < 1e-12), key=lambda x: x[0]
    )
    if not pts:
        return {"steps": [], "dose_ok": False, "damage_pp": 0.0, "passes": False, "by_seed": []}
    ...
    steps = []
    dose_ok = True
    for (r0, v0), (r1, v1) in pairwise(pts):
        m0, sd0, n0 = _stats(v0)
        m1, sd1, n1 = _stats(v1)
        ...
        steps.append(
            {"r_from": r0, "r_to": r1, "cagr_from": m0, "cagr_to": m1, "delta": m1 - m0, "tol": tol, "ok": ok}
        )
    ...
    by_seed = [
        {
            ...
            "spread": (
                max(ps["base_cagr"] for ps in v["per_seed"]) - min(ps["base_cagr"] for ps in v["per_seed"])
                if v["per_seed"]
                else 0.0
            ),
        }
    ...
    return {
        ...
        "damage_pp": damage,
        "damage_ok": damage_ok,
        "passes": bool(dose_ok and damage_ok),
        "by_seed": by_seed,
...
def evaluate_sanity(
    summaries: dict, results: dict, *, repro: dict, window, ruin_mono: dict, ruin_digests_ok: bool
) -> dict:
    orc, rnd = summaries[ORACLE_ARM], summaries[RANDOM_KEEP_ARM]
    s2 = (orc["cagr"] - rnd["cagr"]) >= SANITY_ORACLE_VS_RANDOM_CAGR
    ...
    all_ok = bool(
        ...
        and s2dd
        and all(mech.values())
        and diff >= SANITY_MIN_TRADE_DIFF
        and ruin_mono["passes"]
        and ruin_digests_ok
        and repro_ok
    )
    return {
        "accounting_ok": accounting,
        "oracle_vs_random_cagr": orc["cagr"] - rnd["cagr"],
        "s2_cagr": s2,
        "oracle_vs_random_dd": orc["max_dd"] - rnd["max_dd"],
        "s2_dd": s2dd,
        "mechanical": mech,
        "mechanical_ok": all(mech.values()),
        "trade_diff": diff,
        "s4_bite": diff >= SANITY_MIN_TRADE_DIFF,
        "ruin_monotone": ruin_mono,
        "ruin_digests_ok": ruin_digests_ok,
...
def evaluate(
    ...
    c2 = (
        cand["max_dd"] <= base["max_dd"] + KILL_DD_TOL
    ...
    for rate, depth in c9_points:
        ...
        c9_detail.append(
            {"rate": rate, "depth": depth, "dcagr_worst": (v["dcagr_worst"] if v else None), "ok": ok}
        )
    c9 = all(d["ok"] for d in c9_detail)

    shippable = is_shippable(*wf["star"])
    ship = bool(c1 and c2 and c3 and c4 and c5_ok and c6 and c7 and c8 and c9 and shippable)

    if wf["star_arm"] == BASELINE_ARM:
        outcome = (
            "NO-SHIP y resultado POSITIVO — el walk-forward elige el BASELINE: "
            "el stop duro se gana su lugar y la serie 26→26b→34→37 queda "
            "cerrada (§8, caso partido 3)."
        )
    elif not shippable:
        outcome = (
            "NO-SHIP POR CONSTRUCCIÓN — el walk-forward elige un brazo con el "
        ...
        outcome = (
            f"SHIP — se cabla `atr_trail_mult` (y `atr_hard_stop_enabled` si "
        ...
        outcome = (
            "NO-SHIP por C9 — y es el resultado MÁS INFORMATIVO posible: "
            "significa que la ventaja del candidato ES el survivorship "
            "(§8, caso partido 1)."
        )
    elif c1 and (not c2 or not c6):
        outcome = (
            "NO-SHIP — C1 pasa pero falla C2 o C6: más retorno con más drawdown "
    ...
    return {
        ...
        "shippable": shippable,
        "dcagr_oos": dcagr_oos,
        "dcagr_insample": cand["cagr"] - base["cagr"],
...
def _run(argv: list[str] | None = None) -> int:
    ...
    p.add_argument(
        "--cache-dir", default=None, help="directorio de memoización (permite reanudar la corrida)"
    )
    p.add_argument(
        "--budget-seconds",
        type=float,
        default=None,
        help="corre hasta agotar el presupuesto y sale limpio; volver a invocar retoma donde quedó",
    )
    p.add_argument(
        "--ruin-seeds", type=int, default=None, help="usar menos de 3 semillas (SMOKE — invalida C9)"
    )
    p.add_argument(
        "--no-walkforward",
        action="store_true",
    ...
    smoke = bool(args.no_walkforward or args.no_ruin or args.ruin_seeds or wrong_universe)
    ...
    def _art_stamp() -> str:
        ...
        for d in ("data/parquet", "data/pit_signals"):
            root = _HERE.parent / d
            newest = max((f.stat().st_mtime_ns for f in root.glob("*") if f.is_file()), default=0)
            out.append(f"{d}:{newest}")
        return "|".join(out)

    _load_tag = f"load|{args.universe}|{args.period}|w{args.warmup}|t{len(tickers)}|{_art_stamp()}"
    ...
    print(
        f"Rejilla 2-D: stop {[_m(s) for s in STOP_MULTS]} × "
    ...
    if args.no_walkforward:
        ...
        wf: dict[str, Any] = {
            ...
            "agreement": 0,
            "proc": {
                "cagr": summaries[arm_name(*star)]["cagr"],
                "max_dd": summaries[arm_name(*star)]["max_dd"],
                "final_equity": 0.0,
            },
            "base": {
                "cagr": summaries[BASELINE_ARM]["cagr"],
    ...
    # 4. C5′ + C5′-bis.
    c5 = (
        regime_criterion(results[BASELINE_ARM], results[cand_arm], n_resamples=args.resamples, seed=BOOT_SEED)
        if cand_arm != BASELINE_ARM
        else {
            "passes": True,
            "tolerance_pts": TOL_MATERIAL_PTS,
            "material_pts": TOL_MATERIAL_PTS,
            "detectable_pts": None,
            "pooled_delta_pts": 0.0,
            "pooled_ci_low": None,
            "pooled_ci_high": None,
            "windows": {},
            "escalate_c9": False,
    ...
    if cand_arm != BASELINE_ARM:
        ...
        sb = _sim(
            f"c8a|{BASELINE_ARM}|mp{args.sens_max_positions}|{run_tag}",
            entries,
            bars_by,
            sigs_by,
            **arm_params(BASELINE_STOP, BASELINE_TRAIL),
        ...
        sc = _sim(
            f"c8a|{cand_arm}|mp{args.sens_max_positions}|{run_tag}",
            entries,
            bars_by,
            sigs_by,
        ...
        cb = _sim(
            f"c8b|{BASELINE_ARM}|close|{run_tag}",
            entries,
            bars_by,
            sigs_by,
    ...
    if not args.no_ruin and cand_arm != BASELINE_ARM:
        print("  §3 — barrido de ruina (`gradual`, la que DECIDE) …", file=log, flush=True)
        sweep = ruin_sweep(
            entries,
            bars_by,
            sigs_by,
            common,
            cand_cell,
            points=gradual_points(),
            shape="gradual",
            n_seeds_used=args.ruin_seeds,
            tag=run_tag,
            log=log,
        )
        ...
        sweep_gap = ruin_sweep(
            entries,
            bars_by,
            sigs_by,
            common,
            cand_cell,
            points=list(C9_POINTS),
            shape="gap",
            n_seeds_used=args.ruin_seeds,
            tag=run_tag,
            log=log,
    ...
    sanity = evaluate_sanity(
        summaries, results, repro=repro, window=window, ruin_mono=ruin_mono, ruin_digests_ok=digests_ok
    )
    ...
    if smoke:
        verdict["ship"] = False
        verdict["outcome"] = (
            "SMOKE — corrida de cañería, sin veredicto. Faltan legs congelados del pre-registro."
        )
    elif not sanity["all_ok"]:
        verdict["ship"] = False
        verdict["outcome"] = (
            "CORRIDA INVÁLIDA — falla un sanity del §7; no hay "
    ...
    ctx: dict[str, Any] = {
        ...
        "sens5": sens5,
        "sens_close": sens_close,
        "ruin": sweep,
        "ruin_gap": sweep_gap,
        "breakeven": {f"d{d:.2f}": breakeven_rate(sweep, d) for d, _ in RUIN_GRID} if sweep else {},
        "boot": (
            {
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        print(
            f"    cache: {_CACHE.hits} reusadas · {_CACHE.misses} nuevas en esta invocación.", file=sys.stderr
        )
        print("    Volvé a invocar con los MISMOS argumentos para retomar.", file=sys.stderr)
        return 2
...
def _report(summaries: dict, tails: dict, ctx: dict) -> None:
    ...
    print("=" * W)
    print(
        f"Ventana: {ctx['window']} · {ctx['n_tickers']} tickers · "
    ...
    for n in [arm_name(*c) for c in grid_cells()] + [ORACLE_ARM, RANDOM_KEEP_ARM]:
        ...
        print(
            f"{n:<14}{100 * d['cagr']:8.2f}%{sh:>8}{100 * d['max_dd']:8.1f}%"
    ...
    if wf.get("per_fold"):
        ...
        print("-" * W)
        print(f"{'test OOS':<26}{'n_train':>9}{'n_test':>8}{'elige':>14}{'OOS proc':>10}{'OOS base':>10}")
        for f in wf["per_fold"]:
            print(
                f"{f['test']:<26}{f['n_train']:9d}{f['n_test']:8d}{f['pick']:>14}"
        ...
        print(
            f"Acuerdo: {wf['agreement']}/{len(FOLDS)} folds eligen "
    ...
    if c5.get("windows"):
        ...
        print(
            f"Tolerancia efectiva {c5['tolerance_pts']:.2f} pts = max(material "
            f"{c5['material_pts']:.2f} · detectable "
            f"{'—' if det is None else f'{det:.2f}'}) — computada, no elegida"
        )
        print(f"\n{'ventana':<22}{'n':>7}{'σ':>8}{'detect.':>10}{'Δ pts':>9}{'IC95%':>22}{'':>3}")
        for r in (*REGIMES, "stress_POOLED"):
            w = c5["windows"].get(r)
            if not w:
                continue
            ...
            star = " ← GATE" if r == "stress_POOLED" else (" ← RESUELVE" if w["resolves_negative"] else "")
            print(f"{r:<22}{w['n_base']:7d}{w['sd_pts']:8.2f}{d2:>10}{w['delta_pts']:+9.2f}{ci:>22}{star}")
        print(
    ...
    if ctx.get("trailing_gap"):
        ...
        for n, g in ctx["trailing_gap"].items():
            print(
                f"  {n:<14} {g['share']:6.1%} de {g['n_total']:5d} trades · "
    ...
    if ctx.get("ruin"):
        ...
        if rm.get("by_seed"):
            ...
            for st in rm["steps"]:
                ...
                print(
                    f"    {st['r_from']:.2%}→{st['r_to']:.2%}".ljust(22)
                    + f"{100 * st['delta']:+8.2f}{100 * st['tol']:11.2f}{mark}"
                )

        print("\n  C9 (peor de las semillas ≥ 0):")
        for d in ctx["verdict"]["c9_detail"]:
            dv = "—" if d["dcagr_worst"] is None else f"{100 * d['dcagr_worst']:+.2f} pp"
            print(f"    r={d['rate']:.2%} / d={d['depth']:.0%} → {dv}  {'OK' if d['ok'] else 'FALLA'}")
    ...
    rows = [
        ...
        (
            f"oráculo vs azar igualado ΔCAGR ≥ +1.50 pp ({100 * s['oracle_vs_random_cagr']:+.2f})",
            s["s2_cagr"],
        ),
        (f"oráculo vs azar ΔmaxDD ≤ −5.00 pp ({100 * s['oracle_vs_random_dd']:+.2f})", s["s2_dd"]),
        ...
        ("la inyección es idéntica para todos los brazos (hash)", s["ruin_digests_ok"]),
    ]
    ...
    print("-" * W)
    b = ctx["boot"]
    ci = f"[{100 * b['ci_low']:+.2f}, {100 * b['ci_high']:+.2f}] p={b['p_value']:.3f}" if b else "—"
    checks = [
        ("C1", "ΔCAGR fuera de muestra ≥ +1.00 pp", f"{100 * v['dcagr_oos']:+.2f} pp", v["c1_cagr_oos"]),
        (
            "C2",
            "maxDD in-sample Y OOS ≤ base +1.00 pp",
        ...
        (
            "C5′",
            "régimen con potencia (agregado de stress, IC)",
        ...
        (
            ...
            f"{v['tail_worst_delta']:+.2f} / {v['tail_p1_delta']:+.2f} pp",
            v["c6_tail"],
        ),
        ("C7", "mismo brazo en ≥4/5 folds", f"{wf['agreement']}/{len(FOLDS)}", v["c7_folds"]),
        (
            "C8",
            "signo a 5 slots Y en modo `close`",
            (
                f"{100 * ctx['sens5']['dcagr']:+.2f} / {100 * ctx['sens_close']['dcagr']:+.2f} pp"
                if ctx.get("sens5") and ctx.get("sens_close")
                else "—"
            ),
            v["c8_spec"],
        ),
        (
            "C9",
            "ruina: Δ ≥ 0 en los puntos medidos" + (" + escalón (C5′-bis)" if v["c9_escalated"] else ""),
            " · ".join(
                f"{100 * d['dcagr_worst']:+.2f}" if d["dcagr_worst"] is not None else "—"
                for d in v["c9_detail"]
            ),
            v["c9_ruin"],
        ),
    ]
