...
def build_arms(score_by, realized, *, n_seeds: int = N_SEEDS) -> dict:
    ...
    for k in range(n_seeds):
        ...
        arms[rot_name(k)] = (
            lambda t, d, _s=ROT_SEED_BASE + k: neutral_rank(_s, d, t)
        )
        arms[fix_name(k)] = (
            lambda t, d, _s=FIX_SEED_BASE + k: fixed_rank(_s, t)
        )
    return arms
...
def aligned_daily(results: dict[str, PortfolioResult],
                  arms: list[str]) -> dict[str, list[tuple[str, float]]]:
    ...
    for name in arms:
        ...
        for dt in dates:
            if dt in d:
                last = d[dt]
            filled.append(last)
        out[name] = [(dates[i], filled[i] / filled[i - 1] - 1.0)
                     for i in range(1, len(filled)) if filled[i - 1] > 0]
    return out


def policy_series(daily: dict[str, list[tuple[str, float]]],
...
def rank_autocorr(key, by_date: dict[str, list[str]], lag: int = 1) -> float | None:
    ...
    rhos: list[float] = []
    for prev, cur in zip(dates, dates[lag:]):
...
def evaluate(base: dict, seeds: list[dict], boot, regime_delta: dict,
             sens: dict | None) -> dict:
    ...
    if ship:
        outcome = ("SHIP — el ranking pasa a orden aleatorio rotado "
                   f"(`paper_ranking_mode=neutral_random`, semilla {ROT_SEED_BASE}).")
    elif c1 and not c3:
        outcome = ("NO-SHIP — caso partido resuelto ex ante (§6): la política rinde "
                   "más pero el drawdown se deteriora por encima de la tolerancia "
                   "declarada. Se reporta cuánto drawdown compra el score.")
    elif c1 and not c2:
        outcome = (f"NO-SHIP — C2 falla: ganan {n_win}/{len(cagrs)} semillas. Se cablea "
        ...
        outcome = ("NO-SHIP — el bootstrap pareado no clarea el 95%. Precedente directo "
                   "y del mismo harness: es lo que le pasó al alfabético en la T21.")
    elif c1 and c2 and c3 and c4 and not c5:
        outcome = ("NO-SHIP — C5: la política cuesta retorno de cartera en al menos un "
                   "régimen por encima de la tolerancia declarada.")
    elif c1 and c2 and c3 and c4 and c5 and not c6:
        outcome = ("NO-SHIP — C6: el resultado no sobrevive a 5 slots, donde el ranking "
                   "debería decidir MÁS (peor ratio de selección), no menos.")
    elif not c1:
        outcome = ("NO-SHIP — el déficit del score no alcanza el umbral con la config "
    ...
    return {
        "median_cagr": med_cagr, "min_cagr": cagrs[0] if cagrs else None,
        "max_cagr": cagrs[-1] if cagrs else None, "median_max_dd": med_dd,
        "dcagr": med_cagr - base["cagr"], "dd_delta": med_dd - base["max_dd"],
        "n_win": n_win, "n_seeds": len(cagrs),
        "c1_cagr": c1, "c2_all_seeds": c2, "c3_maxdd": c3,
        "c4_bootstrap": c4, "c5_regime": c5, "c6_sensitivity": c6,
...
def _simulate(entries, bars_by, sigs_by, arms, common, *, log, label) -> dict:
    ...
    for i, (name, fn) in enumerate(arms.items(), 1):
        ...
        out[name] = simulate_portfolio(entries, bars_by, sigs_by, atr_p=AtrParams(),
...
def main(argv: list[str] | None = None) -> int:
    ...
    results = _simulate(entries, bars_by, sigs_by, arms, common,
                        log=log, label=f"{args.max_positions} slots")
    ...
    boot = paired_block_bootstrap([r for _, r in base_daily], [r for _, r in pol],
    ...
    if not args.no_sensitivity:
        ...
        sens_res = _simulate(entries, bars_by, sigs_by, sens_arms, sens_common,
                             log=log, label=f"{args.sens_max_positions} slots")
        ...
        sens = {
            ...
            "base_cagr": s_base, "median_cagr": statistics.median(s_cagrs),
            "min_cagr": s_cagrs[0], "max_cagr": s_cagrs[-1],
    ...
    pair_diffs = [trade_overlap(results[rot_names[i]], results[rot_names[j]])
                  for i in range(len(rot_names)) for j in range(i + 1, len(rot_names))]
    ...
    sanity = {
        ...
        "seeds_effective": (bool(pair_diffs)
                            and statistics.median(pair_diffs) >= SANITY_MIN_TRADE_DIFF
                            and len(set(seed_cagrs)) == len(seed_cagrs)),
    }
    sanity["all_ok"] = bool(sanity["accounting"] and sanity["repro_ok"]
                            and sanity["oracle_ok"] and sanity["anti_oracle_ok"]
                            and sanity["ranking_bites"] and sanity["seeds_effective"])

    verdict = evaluate(summaries[BASELINE_ARM], [summaries[n] for n in rot_names],
                       boot, regime_delta, sens)
    ...
    ctx = {
        ...
        "eval_mode": EVAL_MODE, "fill_mode": HARNESS_FILL_MODE, "live_gates": LIVE_GATES,
        "n_seeds": args.seeds, "shipped_seed": ROT_SEED_BASE,
        "sanity": sanity, "verdict": verdict, "bootstrap": vars(boot),
        "rot_cagr": {"min": rot_cagrs[0], "median": statistics.median(rot_cagrs),
                     "max": rot_cagrs[-1]},
        "fix_cagr": {"min": fix_cagrs[0], "median": statistics.median(fix_cagrs),
                     "max": fix_cagrs[-1]},
        "regime_base": reg_base, "regime_policy": reg_pol, "regime_delta": regime_delta,
        "sensitivity": sens, "persistence": persistence,
...
def _report(summaries, ctx, verdict, sanity, boot, rot_cagrs, fix_cagrs, dsr, pbo, T):
    ...
    print(hdr)
    print("-" * len(hdr))
    for n, tag in [(BASELINE_ARM, "BASE (lo vivo)"), (INVERTED_ARM, "diagnóstico"),
                   (ORACLE_ARM, "sanity"), (ANTI_ORACLE_ARM, "sanity")]:
        s = summaries[n]
        print(f"{n:<16}{_f(s['cagr'],10,2,'%')}{_f(s['sharpe'],9,2)}{_f(s['max_dd'],9,1,'%')}"
              f"{s['n_taken']:>8}{s['mean_held_days']:>7.1f}  {tag}")
    print(f"{'N_rot ×' + str(ctx['n_seeds']):<16}{_f(ctx['rot_cagr']['median'],10,2,'%')}"
          f"{'':>9}{_f(verdict['median_max_dd'],9,1,'%')}{'':>8}{'':>7}  "
          f"*CANDIDATO (la política) [{100*rot_cagrs[0]:.2f}%, {100*rot_cagrs[-1]:.2f}%]")
    print(f"{'P_fix ×' + str(ctx['n_seeds']):<16}{_f(ctx['fix_cagr']['median'],10,2,'%')}"
    ...
    print(f"  [{'OK' if sanity['seeds_effective'] else 'FALLA'}] las semillas mueven: "
    ...
    if pers:
        ...
        if lags:
            print("  el orden del score a k ruedas: "
    ...
    for r in [BULL_NORMAL] + [x.name for x in STRESS_REGIMES]:
        print(f"  {r:<20} base {100*ctx['regime_base'][r]:>+8.2f}% · "
    ...
    print(f"Bootstrap pareado (serie promedio de las semillas): ΔCAGR obs "
    ...
    if ctx["sensitivity"]:
        s = ctx["sensitivity"]
        print(f"Sensibilidad a {s['max_positions']} slots: base {100*s['base_cagr']:.2f}% · "
              f"mediana {100*s['median_cagr']:.2f}% [{100*s['min_cagr']:.2f}%, "
              f"{100*s['max_cagr']:.2f}%] · ganan {s['n_win']}/{ctx['n_seeds']}")

    print("\nRegla de decisión (§6):")
    for k, label in [("c1_cagr", "C1 ΔCAGR mediana ≥ +0.50pp"),
